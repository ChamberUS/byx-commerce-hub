...
// Try to load local images, fallback handled via onError
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row bg-background">
      ...
      <div className="relative lg:w-1/2 lg:min-h-screen overflow-hidden">
        ...
        {/* Gradient overlay (always present as fallback) */}
        ...
        <div className="relative z-10 flex items-center justify-center h-full px-8 py-12 lg:px-14 xl:px-20 min-h-[240px] lg:min-h-screen">
          <div className="w-full max-w-xl">
            <AnimatePresence mode="wait" custom={direction}>
              ...
              >
                <span className="inline-block text-xs font-bold tracking-[0.2em] uppercase text-white/50 mb-4 lg:mb-6">
                  Buynnex
                </span>
                <h1 className="text-2xl lg:text-4xl xl:text-[2.75rem] font-bold text-white leading-tight mb-3">
                  {panel.headline}
                </h1>
                <p className="text-sm lg:text-base text-white/75 leading-relaxed mb-8 lg:mb-10 max-w-md">
                  {panel.sub}
                </p>
          ...
          <div className="flex rounded-xl bg-muted p-1 mb-8" role="tablist">
            {(['signin', 'signup'] as Mode[]).map((m) => (
              <button
                ...
                className={`flex-1 py-2.5 text-sm font-semibold rounded-lg transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 ${
                  mode === m
              ...
              {stage === 'input' ? (
                <>
                  ...
                  <form onSubmit={handleSubmit} className="space-y-5">
                    <div className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        <Input
                          ...
                          className={`pl-11 h-11 min-h-[44px] rounded-xl text-base ${error ? 'border-destructive focus-visible:ring-destructive' : ''}`}
                          autoComplete="email"
                          autoFocus
                          maxLength={254}
                        />
                    ...
                    <Button
                      ...
                      className="w-full h-11 min-h-[44px] rounded-xl text-base font-semibold"
                      size="lg"
                    >
                      {loading ? (
                  ...
                  <div className="mt-6 pt-5 border-t border-border text-center space-y-2">
                    <p className="text-sm text-muted-foreground">
                      {mode === 'signin' ? (
                        <>Não tem uma conta?{' '}
                          <button onClick={() => switchMode('signup')} className="text-primary font-semibold hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring rounded">
                            Criar conta
                          </button>
                        </>
                      ) : (
                        <>Já tem uma conta?{' '}
                          <button onClick={() => switchMode('signin')} className="text-primary font-semibold hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring rounded">
                            Entrar
                          </button>
                        </>
                      )}
                    ...
                    {mode === 'signin' && seenBefore && (
                      <button
                        onClick={() => { localStorage.removeItem(SEEN_KEY); setSeenBefore(false); setEmail(''); }}
                ...
                <div className="text-center py-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <Button variant="ghost" onClick={resetEmail} className="w-full h-11 min-h-[44px] rounded-xl text-muted-foreground">
